...
import {
  simuladorPersona,
  simuladorBox,
  simuladorFuncionario,
  simuladorAgendamiento,
  simuladorResultadosConsulta,
} from './simulador';
...
export class AppService {
  ...
  // Posts/Simulacion
  async postPaciente(): Promise<modelos.Persona> {
    // Postea un paciente, retorna una persona
    const persona = simuladorPersona();
    try {
      const query =
        'INSERT INTO paciente(nombre, apellido, rut) VALUES ($1, $2, $3)';
  ...
  async postBox(): Promise<modelos.Box> {
    // Postea un box, retorna un box
    const box = simuladorBox();
    try {
      const query =
        'INSERT INTO box(idEspecialidadBox, pasillo) VALUES ($1, $2)';
  ...
  async postFuncionario(): Promise<modelos.Funcionario> {
    // Postea un funcionario
    const funcionario = simuladorFuncionario();
    try {
      const query =
        'INSERT INTO funcionario(idTipoFuncionario, nombre, rut) VALUES ($1, $2, $3) RETURNING idfuncionario';
      const values = [
  ...
  async postAgendamiento(): Promise<object> {
    // Post de agendamiento y estadoagendamiento
    let jsonRes: Record<string, any> = {}
    const agendamiento = await simuladorAgendamiento();
  ...
  // APIs solicitadas por el tomah
  async getAgendamientoEspecialidad(especialidad: string,): Promise<object> {
    // Obtener agendamientos por especialidad
    const jsonAgendamientos: Record<string, any> = {}
    ...
    res.rows.forEach((element) => {
      let temp = new modelos.Agendamiento(
        element.idagendamiento,
        element.idbox,
        element.idfuncionario,
        element.idpaciente,
        element.fecha,
        element.horaentrada,
        element.horasalida,
  ...
  async getCountAgendamientosPorEspecialidad(): Promise<Array<modelos.Agendamiento>> {
    const listaAgendamientos: any[] = [];
    const query = `SELECT 
  ...
  async getCountAgendamientosPorEspecialidadRangoFechas(
    fecha_inicio: string,
    fecha_final: string,
  ): Promise<Array<modelos.Agendamiento>> {
    const listaAgendamientos: any[] = [];
    const query = `SELECT 
          especialidadbox.especialidad as especialidad,
          COUNT(*) AS total_agendamientos
  ...
  async getAgendamientoFecha(
    ...
    let listaAgendamientos: modelos.Agendamiento[] = [];
  ...
  async getAgendamientoEspecialidadFecha(
    ...
    if (idEspecialidadBox != 0) {
      const query =
        'SELECT idagendamiento,agendamiento.idbox,idfuncionario,idpaciente,fecha,horaentrada,horasalida FROM agendamiento INNER JOIN box ON agendamiento.idbox = box.idbox WHERE idespecialidadbox = $1 AND fecha BETWEEN $2 AND $3';
      ...
      throw error('Especialidad no existente');
    }
  }
  ...
  async postAgendamientoPersonalizado(
    ...
    // verificador de ids aca (no lo wa hacer ahora salu2)
  ...
  // APIs de autenticación de usuarios y creación de usuarios
  async postUsuario(
    nombre: string,
    contraseña: string,
    rol: string,
    adicional?: any,
  ): Promise<modelos.Usuario> {
    ...
    if (typeof adicional == undefined) {
      ...
      try {
        ...
        const query =
          'INSERT INTO usuarios(nombre, contraseña, rol, informacionadicional) VALUES ($1, $2, $3, $4)';
        const values = [
          usuario.nombre,
          usuario.contraseña,
          usuario.rol,
  ...
  async getUsuario(nombre: string): Promise<modelos.Usuario> {
    const query = 'SELECT * FROM usuarios WHERE nombre = $1';
    const values = [nombre];
    const res = await pool.query(query, values);
    const usuario = new modelos.Usuario(
      res.rows[0].nombre,
      res.rows[0].contraseña,
      res.rows[0].rol,
      res.rows[0].informacionadicional,
    );
    return usuario;
  }

  async verificador(nombre: string, contraseña: string): Promise<boolean> {
  ...
  async Query1(fecha1:string, fecha2:string): Promise<object> {
    ...
    const fechasJson: Record<string, number> = {};
  ...
  async postAgendamientoSUPREMUS(fecha:string, horaEntrada:string, horaSalida:string): Promise<boolean> {
    let jsonRes: Record<string, any> = {}
    const agendamiento = await simuladorAgendamiento();
  ...
  async disponibilidad(box:number, fecha:string): Promise<Array<string>> {
    // Horas disponibles en un día de un box especifico
  ...
  async eliminarAgendamiento(id:number): Promise<boolean> {
  ...
  async usoBox(idbox:string, fecha:string, hora:string): Promise<object> {
    // id, ocupancia, especialidad, libre ahora?, proxima hora : Por box
    let jsonBox: Record<string, any> = {}
    try{
  ...
  async agendamientosPorFecha(fecha:string): Promise<object>{

    let jsonAgendamientos: Record<string, any> = {}
  ...
  async updateEstadoSimulaConsulta(idagendamiento:number): Promise<object> {
    let jsonRes: Record<string, any> = {}
    ...
    try {
      ...
      const fechaTemp: Date = temp.fecha
      ...
      const resultadosConsulta = await simuladorResultadosConsulta(agendamiento)
  ...
  async updateEstado(idagendamiento:number, nuevoEstado:string): Promise<object>{
    let jsonRes: Record<string, any> = {}
  ...
  async agendamientosPorConfirmar(): Promise<Array<modelos.Agendamiento>> {
    const array: Array<modelos.Agendamiento> = []
  ...
  async boxesLibres(fecha:string, hora:string):Promise<Array<modelos.Box>> {
    const array: Array<modelos.Box> = []
  ...
  async cambioAgendamiento(fecha1:string, fecha2: string): Promise<object> {
    const jsonRes: Record<string, any> = {}
  ...
  async agendamientoTotalHoyBox(idbox: number, fecha: string): Promise<object> {
  ...
  async agendamientosPendientesBox(idbox: number, fecha: string): Promise<object> {
  ...
  async proximoAgendamientoBox(idbox: number, fecha: string, hora:string): Promise<string> {
  ...
  async cantidadAgendamientosEntreFechas(fecha1: string, fecha2:string): Promise<number> {
  ...
  async pacientesNoAtendidos(fecha:string) : Promise<object> {
  ...
  async pacientesAtendidos(fecha:string) : Promise<object> {
  ...
  async diferenciaOcupanciaMeses(mes1:string, mes2:string): Promise<number> {
    const query1 = "SELECT COUNT(*) FROM agendamiento WHERE fecha >= $1 AND fecha <= $2"
    ...
    try {
      ...
      return (res2.rows[0].count*30)/(930*dias2*totalBoxes.rows[0].count) - (res.rows[0].count*30)/(930*dias1*totalBoxes.rows[0].count) // (totalAgendamientos*minutosQueDuraUnBloque)/(totalMaximoMinutosEnUnDia*totalDiasMes*cantidadBoxes)
    }catch (error) {
      throw error
    }
  }
